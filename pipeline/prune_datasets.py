...
1. Filter the datsets using labels, particularly the step:init_datasets (only
created by baby_pipeline)
...
def prune_datasets(argv):
    ...
    get_datasets = client.list_datasets(
        filter="labels.step:init_datasets"
    )  # Make an API request.
    if not get_datasets:
        logging.info("Baby-pipeline datasets to remove not found.")
        return 0
    ...
    datasets_to_remove = list(
        filter(lambda x: x[:PATTERN_LEN] in pattern_to_remove, datasets)
    )
